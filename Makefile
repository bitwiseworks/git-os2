...
ifeq ($(uname_S),OS/2)
	# The default setup assumes the RPM build environment with GCC
	# (based on config.mak.autogen generated by configure), the following
	# extra packages are expected: libcurl-devel openssl-devel gettext-devel
	# expat-devel perl-ExtUtils-MakeMaker
	pathsep = ;
	HOME := $(subst \,/,$(HOME))
	SHELL_PATH = sh
	PERL_PATH = perl
	...
	BASIC_CFLAGS += -g -Zomf
	BASIC_LDFLAGS += -Zbin-files -Zhigh-mem -Zomf -Zargs-wild -Zargs-resp
	EXTLIBS += -lmmap
	X = .exe
endif
